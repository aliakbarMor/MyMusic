...
@AndroidEntryPoint
class MusicWidgetBig : BaseWidget() {
    ...
    override fun onUpdate(
        ...
        for (appWidgetId in appWidgetIds) {
            updateMusicWidget(
                ...
                appWidgetManager,
                appWidgetId,
                RemoteViews(context.packageName, R.layout.music_widget_big)
            )
        }
    }
    ...
    private var musicReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            val remoteViews = RemoteViews(context.packageName, R.layout.music_widget_big)
            when (intent.action) {
                MusicService.ACTION_MUSIC_STARTED -> {
                    ...
                    val appWidgetIds = AppWidgetManager.getInstance(context).getAppWidgetIds(
                        ComponentName(context, MusicWidgetBig::class.java)
                    )
                    for (appWidgetId in appWidgetIds) {
                        this@MusicWidgetBig.updateMusicWidget(
                            context, AppWidgetManager.getInstance(context), appWidgetId, remoteViews
                        )
                        this@MusicWidgetBig.updateIcPlayAndPause(
